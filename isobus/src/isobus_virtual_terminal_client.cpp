...
namespace isobus
{
	  ...
	  txFlags(static_cast<std::uint32_t>(TransmitFlags::NumberFlags), process_flags, this)
	{
		if (nullptr != partnerControlFunction)
		{
			...
			{
				network->add_global_parameter_group_number_callback(static_cast<std::uint32_t>(CANLibParameterGroupNumber::VirtualTerminalToECU), process_rx_message, this);
				network->add_global_parameter_group_number_callback(static_cast<std::uint32_t>(CANLibParameterGroupNumber::ECUtoVirtualTerminal), process_rx_message, this);
			}
		}
	}
	...
	void VirtualTerminalClient::terminate()
	{
		if (initialized)
		{
			if (nullptr != partnerControlFunction)
			{
				...
				if (auto network = partnerControlFunction->get_associated_network().lock())
				{
					network->remove_global_parameter_group_number_callback(static_cast<std::uint32_t>(CANLibParameterGroupNumber::VirtualTerminalToECU), process_rx_message, this);
					network->remove_global_parameter_group_number_callback(static_cast<std::uint32_t>(CANLibParameterGroupNumber::ECUtoVirtualTerminal), process_rx_message, this);
				}
			}
	...
	bool VirtualTerminalClient::send_hide_show_object(std::uint16_t objectID, HideShowObjectCommand command) const
	{
		...
		return CANNetworkManager::send_can_message(static_cast<std::uint32_t>(CANLibParameterGroupNumber::ECUtoVirtualTerminal),
		                                           buffer.data(),
		                                           CAN_DATA_LENGTH,
		                                           myControlFunction,
	...
	bool VirtualTerminalClient::send_change_string_value(std::uint16_t objectID, uint16_t stringLength, const char *value) const
	{
		...
		if (nullptr != value)
		{
			...
			retVal = CANNetworkManager::send_can_message(static_cast<std::uint32_t>(CANLibParameterGroupNumber::ECUtoVirtualTerminal),
			                                             buffer.data(),
			                                             buffer.size(),
			                                             myControlFunction,
			                                             partnerControlFunction,
					...
					for (std::uint32_t i = 0; i < objectPools.size(); i++)
					{
						    ...
						    (objectPools[i].objectPoolSize > 0))
						{
							...
							if (CurrentObjectPoolUploadState::Uninitialized == currentObjectPoolState)
							{
								if (!objectPools[i].uploaded)
								{
									bool transmitSuccessful = CANNetworkManager::send_can_message(static_cast<std::uint32_t>(CANLibParameterGroupNumber::ECUtoVirtualTerminal),
									                                                              nullptr,
	...
	bool VirtualTerminalClient::update_auxiliary_input_status(std::uint16_t objectID)
	{
		     ...
		     SystemTiming::time_expired_ms(state.lastStatusUpdate, AUXILIARY_INPUT_STATUS_DELAY_INTERACTION)))
		{
			...
			{
				retVal = CANNetworkManager::send_can_message(static_cast<std::uint32_t>(CANLibParameterGroupNumber::VirtualTerminalToECU),
				                                             buffer.data(),
				                                             CAN_DATA_LENGTH,
				                                             myControlFunction,
				                                             nullptr,
