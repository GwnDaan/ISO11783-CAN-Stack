...
namespace isobus
{
	ExtendedTransportProtocolManager::ExtendedTransportProtocolSession::ExtendedTransportProtocolSession(Direction sessionDirection) :
	...
	void ExtendedTransportProtocolManager::initialize(std::shared_ptr<CANNetworkManager> network, CANLibBadge<CANNetworkManager>)
	{
		if (!initialized)
		{
			initialized = true;
			network->add_protocol_parameter_group_number_callback(static_cast<std::uint32_t>(CANLibParameterGroupNumber::ExtendedTransportProtocolDataTransfer), process_message, this);
			network->add_protocol_parameter_group_number_callback(static_cast<std::uint32_t>(CANLibParameterGroupNumber::ExtendedTransportProtocolConnectionManagement), process_message, this);
		}
	}

	void ExtendedTransportProtocolManager::process_message(const CANMessage &message)
	{
		if ((nullptr != std::dynamic_pointer_cast<InternalControlFunction>(message.get_destination_control_function())))
		{
					...
					if (CAN_DATA_LENGTH == message.get_data_length())
					{
						...
						switch (message.get_data()[0])
						{
							case EXTENDED_REQUEST_TO_SEND_MULTIPLEXOR:
							{
								...
								{
									ExtendedTransportProtocolSession *newSession = new ExtendedTransportProtocolSession(ExtendedTransportProtocolSession::Direction::Receive);
								...
								{
									abort_session(pgn, ConnectionAbortReason::AlreadyInConnectionManagedSessionAndCannotSupportAnother, std::dynamic_pointer_cast<InternalControlFunction>(message.get_destination_control_function()), message.get_source_control_function());
									...
									close_session(session, false);
								         ...
								         (ControlFunction::Type::Internal == message.get_destination_control_function()->get_type()))
								{
									abort_session(pgn, ConnectionAbortReason::SystemResourcesNeededForAnotherTask, std::dynamic_pointer_cast<InternalControlFunction>(message.get_destination_control_function()), message.get_source_control_function());
							...
							case EXTENDED_CLEAR_TO_SEND_MULTIPLEXOR:
							{
								...
								if (get_session(session, message.get_destination_control_function(), message.get_source_control_function(), pgn))
								{
									...
									else
									{
										...
										abort_session(pgn, ConnectionAbortReason::ClearToSendReceivedWhenDataTransferInProgress, std::dynamic_pointer_cast<InternalControlFunction>(message.get_destination_control_function()), message.get_source_control_function());
								...
								else
								{
									...
									// In the case of Rx'ing a CTS, we're the source in the session
									abort_session(pgn, ConnectionAbortReason::AnyOtherReason, std::dynamic_pointer_cast<InternalControlFunction>(message.get_destination_control_function()), message.get_source_control_function());
							...
							case EXTENDED_DATA_PACKET_OFFSET_MULTIPLEXOR:
							{
								...
								else
								{
									...
									{
										abort_session(pgn, ConnectionAbortReason::UnexpectedEDPOPacket, std::dynamic_pointer_cast<InternalControlFunction>(message.get_destination_control_function()), message.get_source_control_function());
									}
								}
							}
							break;
				...
				case static_cast<std::uint32_t>(CANLibParameterGroupNumber::ExtendedTransportProtocolDataTransfer):
				{
					    ...
					    (messageData[SEQUENCE_NUMBER_DATA_INDEX] == (tempSession->lastPacketNumber + 1)))
					{
						...
						if ((tempSession->processedPacketsThisSession * PROTOCOL_BYTES_PER_FRAME) >= tempSession->get_message_data_length())
						{
							...
							if (auto network = message.get_source_control_function()->get_associated_network().lock())
							{
								network->process_any_control_function_pgn_callbacks(tempSession->sessionMessage);
								network->protocol_message_callback(tempSession->sessionMessage);
							}
	...
	bool ExtendedTransportProtocolManager::abort_session(ExtendedTransportProtocolSession *session, ConnectionAbortReason reason)
	{
		...
		if (nullptr != session)
		{
			...
			{
				myControlFunction = std::dynamic_pointer_cast<InternalControlFunction>(session->sessionMessage.get_source_control_function());
				partnerControlFunction = session->sessionMessage.get_destination_control_function();
			}
			else
			{
				myControlFunction = std::dynamic_pointer_cast<InternalControlFunction>(session->sessionMessage.get_destination_control_function());
			...
			retVal = CANNetworkManager::send_can_message(static_cast<std::uint32_t>(CANLibParameterGroupNumber::ExtendedTransportProtocolConnectionManagement),
			                                             data.data(),
			                                             CAN_DATA_LENGTH,
			                                             myControlFunction,
	...
	bool ExtendedTransportProtocolManager::abort_session(std::uint32_t parameterGroupNumber, ConnectionAbortReason reason, std::shared_ptr<InternalControlFunction> source, std::shared_ptr<ControlFunction> destination)
	{
		...
		return CANNetworkManager::send_can_message(static_cast<std::uint32_t>(CANLibParameterGroupNumber::ExtendedTransportProtocolConnectionManagement),
		                                           data.data(),
		                                           CAN_DATA_LENGTH,
		                                           source,
		                                           destination,
					...
					if (nullptr != session->sessionMessage.get_destination_control_function())
					{
						...
						if (proceedToSendDataPackets)
						{
							...
							for (std::uint32_t i = session->lastPacketNumber; i < session->packetCount; i++)
							{
								...
								if (CANNetworkManager::send_can_message(static_cast<std::uint32_t>(CANLibParameterGroupNumber::ExtendedTransportProtocolDataTransfer),
								                                        dataBuffer,
								                                        CAN_DATA_LENGTH,
								                                        std::dynamic_pointer_cast<InternalControlFunction>(session->sessionMessage.get_source_control_function()),
								                                        session->sessionMessage.get_destination_control_function(),
