...
namespace isobus
{
	...
	{
	public:
		/// @brief Returns the number of configured network channels that the class is managing
		/// @returns The number of configured network channels that the class is managing
		static std::size_t get_number_of_can_channels();

		/// @brief Assigns a CAN driver to a channel
		/// @note If the hardware interface is already started, it will also immediately start this frame handler
		/// @param[in] network The network to assign the driver to
		...
		static bool assign_can_channel_frame_handler(std::shared_ptr<const CANNetworkManager> network, std::shared_ptr<CANHardwarePlugin> canDriver);

		/// @brief Removes a CAN driver from a network channel
		/// @note If the hardware interface is already started, it will also immediately stop this frame handler
		/// @param[in] network The network to unassign the driver from
		/// @returns `true` if the driver was removed from the channel, otherwise `false`
		static bool unassign_can_channel_frame_handler(std::shared_ptr<const CANNetworkManager> network);
		...
		/// @param[in] network The network to transmit the message from
		...
		static bool transmit_can_frame(std::shared_ptr<const CANNetworkManager> network, const isobus::CANMessageFrame &frame);
	...
	private:
		...
		/// @param[in] network The associated network for the thread
		static void receive_can_frame_thread_function(std::weak_ptr<const CANNetworkManager> network);
		...
		static std::unordered_map<std::shared_ptr<const CANNetworkManager>, std::unique_ptr<CANHardware>> hardwareChannels; ///< A list of all CAN channel's metadata
